...
pub fn decode_deposit(block_hash: B256, index: usize, log: &Log) -> Result<Bytes, DepositError> {
    ...
    let opaque_content_offset: U256 = U256::try_from_be_slice(&log.data.data[0..32]).ok_or(
        DepositError::InvalidOpaqueDataOffset(Bytes::copy_from_slice(&log.data.data[0..32])),
    )?;
    ...
    let opaque_content_len: U256 = U256::try_from_be_slice(&log.data.data[32..64]).ok_or(
        DepositError::InvalidOpaqueDataLength { expected: 64, actual: log.data.data.len() },
    )?;
    ...
    let Some(opaque_data) = &log.data.data.get(64..64 + opaque_content_len as usize) else {
        return Err(DepositError::InvalidOpaqueDataLength {
            expected: 64 + opaque_content_len as usize,
    ...
    if !log
        .data
        .data
        .get(64 + opaque_content_len as usize..64 + 32)
